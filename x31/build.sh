...
# Config: uefi + glibc + gnu + bash + dinit + portage + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + doas + harden + hybrid
...
security_install() {
  msg 'Security: Firewall + hardening'
