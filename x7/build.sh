...
# Config: uefi + glibc + gnu + bash + dinit + portage + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + sudo + firewall + hybrid
...
security_install() {
  msg 'Security: Firewall (nftables)'
