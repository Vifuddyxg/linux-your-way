...
# Config: uefi + glibc + gnu + bash + dinit + xbps + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + doas + apparmor + hybrid
# [experimental] AppArmor on Void: package exists, profile coverage is thin
