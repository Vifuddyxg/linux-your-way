...
# Config: uefi + musl + gnu + bash + dinit + apk + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + sudo + apparmor + hybrid
# [manual-config-needed] no automated AppArmor path here; install profiles and lsm= by hand
# [manual-config-needed] Alpine services are OpenRC scripts; dinit needs hand-written services
...
AMIRROR=${AMIRROR:-https://dl-cdn.alpinelinux.org/alpine/latest-stable}
...
pkg() {
  ./sbin/apk.static -X "$AMIRROR/main" -X "$AMIRROR/community" \
...
base_install() {
  msg 'Base: Alpine via apk.static'
...
EOF
  # MANUAL: install apparmor + profiles yourself
  svc_enable apparmor
...
locale_time() {
  ...
  # musl/uClibc: no locale generation; LANG left to the user
}
