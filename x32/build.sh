...
# Config: uefi + glibc + gnu + bash + dinit + pacman + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + doas + apparmor + hybrid
...
base_install() {
  ...
  basestrap "$ROOT" base dinit elogind-dinit linux-firmware
}
...
locale_time() {
  ...
  sed -i "s/^#$LOC/$LOC/" "$ROOT/etc/locale.gen" || true
