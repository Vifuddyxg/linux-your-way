...
# Config: uefi + glibc + gnu + bash + dinit + portage + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + doas + apparmor + hybrid
# [manual-config-needed] no official stage3 for this init; start from the OpenRC stage3 and swap manually
...
cmdline() {
  printf 'root=UUID=%s rw lsm=landlock,lockdown,yama,integrity,apparmor,bpf' "$(blkid -s UUID -o value "$ROOTP")"
}
...
pkg() {
  # long builds; add --getbinpkg to use the official Gentoo binhost
...
base_install() {
  msg 'Base: Gentoo stage3 (openrc)'
  GMIRROR=${GMIRROR:-https://distfiles.gentoo.org/releases/amd64/autobuilds}
  STAGE=$(curl -s "$GMIRROR/latest-stage3-amd64-openrc.txt" \
...
kernel_install() {
  msg 'Kernel: sys-kernel/gentoo-kernel-bin sys-kernel/linux-firmware (precompiled)'
  pkg sys-kernel/gentoo-kernel-bin sys-kernel/linux-firmware
}
...
bootloader_install() {
  msg 'Bootloader: GRUB'
  pkg sys-boot/grub sys-boot/os-prober
  ...
  echo 'GRUB_CMDLINE_LINUX_DEFAULT="loglevel=3 quiet lsm=landlock,lockdown,yama,integrity,apparmor,bpf"' >> "$ROOT/etc/default/grub"
...
toolchain_install() {
  ...
  # GCC + Binutils already come with the stage3
  printf 'CFLAGS="-O2 -pipe"\nCXXFLAGS="-O2 -pipe"\n' > "$ROOT/etc/lyw-build.conf"
  sed -i 's/^COMMON_FLAGS=.*/COMMON_FLAGS="-O2 -pipe"/' "$ROOT/etc/portage/make.conf"
}

security_install() {
  msg 'Security: AppArmor'
  pkg net-firewall/nftables
  cat > "$ROOT/etc/nftables.conf" <<'EOF'
#!/usr/sbin/nft -f
flush ruleset
table inet filter {
  chain input {
...
EOF
  pkg sys-apps/apparmor sys-apps/apparmor-utils sec-policy/apparmor-profiles
  svc_enable apparmor
...
locale_time() {
  ...
  echo "$LOC UTF-8" >> "$ROOT/etc/locale.gen"
