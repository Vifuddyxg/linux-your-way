...
# Config: uefi + glibc + gnu + bash + dinit + portage + lts + precompiled + gcc + zstd + modules + separate + gnu + compat + none + grub + off + ext4 + none + dhcpcd + password + none + selinux + hybrid
# [manual-config-needed] SELinux on Gentoo: switch to an SELinux profile, install policies, relabel — all manual
